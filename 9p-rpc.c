...
wait_for_header (struct p9_response_interest *interest)
{
 ...
 read_our_header:
  ...
  nread = fread (&size, sizeof (size), 1, p9_conn_in);
  if (nread != 1)
    goto out;
  size = le32toh (size);

  nread = fread (&type, sizeof (type), 1, p9_conn_in);
  if (nread != 1)
    goto out;

  nread = fread (&tag, sizeof (tag), 1, p9_conn_in);
  if (nread != 1)
    goto out;
  tag = le16toh (tag);
...
p9_vserialized_size (const char *format, va_list ap)
{
  ...
  for (f = format; *f; f++)
    switch (*f)
      ...
      case 'S':
        arr = va_arg (ap, const char * const *);
        while (*arr)
          {
            total_size += 2;
            total_size += strchrnul (*arr, '/') - *arr;
            arr++;
          }
        break;
  ...
  for (f = format; *f; f++)
    switch (*f)
      ...
      case '1':
        ...
        nwritten = fwrite (&n1, 1, 1, p9_conn_out);
        if (nwritten != 1)
          return 1;
        break;
      ...
      case '2':
        ...
        n2 = htole16 (n2);
        nwritten = fwrite (&n2, 2, 1, p9_conn_out);
        if (nwritten != 1)
          return 1;
        break;
      ...
      case '4':
        ...
        n4 = htole32 (n4);
        nwritten = fwrite (&n4, 4, 1, p9_conn_out);
        if (nwritten != 1)
          return 1;
        break;
      ...
      case '8':
        ...
        n8 = htole64 (n8);
        nwritten = fwrite (&n8, 8, 1, p9_conn_out);
        if (nwritten != 1)
          return 1;
        break;
      ...
      case 'd':
        ...
        if (nwritten != 1)
          return 1;
        nwritten = fwrite (ptr, 1, size, p9_conn_out);
        if (nwritten != size)
          return 1;
        break;
      ...
      case 'S':
        ...
        while (*arr)
          {
            size = strchrnul (*arr, '/') - *arr;
            assert_backtrace (size <= UINT16_MAX);
            if (p9_dump_exchanges)
              fprintf (stderr, " %" PRIu32 " \"%.*s\"", size, size, *arr);
            n2 = htole16 (size);
            nwritten = fwrite (&n2, 2, 1, p9_conn_out);
            if (nwritten != 1)
              return 1;
            nwritten = fwrite (*arr, 1, size, p9_conn_out);
            if (nwritten != size)
              return 1;
            arr++;
          }
          ...
          const char *format, va_list *ap)
{
  ...
  nwritten = fwrite (&size, sizeof (size), 1, p9_conn_out);
  if (nwritten != 1)
    goto out;

  nwritten = fwrite (&type, 1, 1, p9_conn_out);
  if (nwritten != 1)
    goto out;

  nwritten = fwrite (&tag, sizeof (tag), 1, p9_conn_out);
  if (nwritten != 1)
    goto out;

  /* Write out the body.  */
  rc = p9_do_vsend (format, ap);

out:
  pthread_mutex_unlock (&write_mutex);
  err = ferror (p9_conn_out);
  if (!err && rc)
    err = EIO;
  if (!err)
    fflush (p9_conn_out);
  ...
  for (f = format; *f; f++)
    {
      ...
      switch (*f)
        ...
        case '1':
          ...
          nread = fread (n1p, 1, 1, p9_conn_in);
          if (nread != 1)
            goto out;
          bytes_read++;
          if (p9_dump_exchanges)
        ...
        case '2':
          ...
          nread = fread (n2p, 2, 1, p9_conn_in);
          if (nread != 1)
            goto out;
          bytes_read += 2;
          *n2p = le16toh (*n2p);
        ...
        case '4':
          ...
          nread = fread (n4p, 4, 1, p9_conn_in);
          if (nread != 1)
            goto out;
          bytes_read += 4;
          *n4p = le32toh (*n4p);
        ...
        case '8':
          ...
          nread = fread (n8p, 8, 1, p9_conn_in);
          if (nread != 1)
            goto out;
          bytes_read += 8;
          *n8p = le64toh (*n8p);
        ...
        case 'd':
          ...
          *n4p = le32toh (*n4p);
          ...
          ptr = va_arg (ap, char *);
          nread = fread (ptr, 1, *n4p, p9_conn_in);
          if (nread != *n4p)
            goto out;
          bytes_read += *n4p;
          break;
        ...
        case 's':
          dptr = va_arg (ap, char **);
          nread = fread (&string_size, 2, 1, p9_conn_in);
          if (nread != 1)
            goto out;
          bytes_read += 2;
          ...
          *dptr = malloc (string_size + 1);
          ...
          nread = fread (*dptr, 1, string_size, p9_conn_in);
        ...
        case 'Q':
          ...
          *n2p = le16toh (*n2p);
          if (p9_dump_exchanges)
            fprintf (stderr, " %" PRIu16, *n2p);
          *qbuf = calloc (*n2p, sizeof (struct p9_qid));
          if (!*qbuf)
            {
              err = errno;
              goto out;
            }
          for (qptr = *qbuf; qptr < *qbuf + *n2p; qptr++)
            {
              nread = fread (&qptr->type, 1, 1, p9_conn_in);
              if (nread != 1)
                goto out;
              bytes_read++;
              if (p9_dump_exchanges)
                fprintf (stderr, " %" PRIu8, qptr->type);

              nread = fread (&qptr->version, 4, 1, p9_conn_in);
              if (nread != 1)
                goto out;
              bytes_read += 4;
              qptr->version = le32toh (qptr->version);
              if (p9_dump_exchanges)
                fprintf (stderr, " %" PRIu32, qptr->version);

              nread = fread (&qptr->path, 8, 1, p9_conn_in);
              if (nread != 1)
                goto out;
              bytes_read += 8;
              qptr->path = le64toh (qptr->path);
              if (p9_dump_exchanges)
                fprintf (stderr, " %" PRIu64, qptr->path);
            }
          break;
...
out:
  if (!ferror (p9_conn_in) && bytes_read < interest->size)
    {
      ...
      fread (buffer, 1, interest->size - bytes_read, p9_conn_in);
    }
